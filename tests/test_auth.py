...
from ..route_definitions import IGNORED_ROUTES, PUBLIC_ROUTES, UNPROTECTED_ROUTES
from . import login
...
def test_login_required(client, app):
    with app.test_request_context():
        routes = [
            (url_for(rule.endpoint) if "GET" in rule.methods and rule.endpoint != "static" and rule.endpoint not in IGNORED_ROUTES else "") for rule in app.url_map.iter_rules()
        ]
